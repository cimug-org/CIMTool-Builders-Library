...
| [csharp-ef-rdfs.xsl](shipped-builders/csharp-ef-rdfs/builder.md) | C# source file | The **[csharp-ef-rdfs.xsl](shipped-builders/csharp-ef-rdfs/builder.md)** builder generates a C# source file (`*.csharp-ef-rdfs.cs`) containing Entity Framework Core entity classes. Designed as a companion to the **sql-rdfs-ansi92.xsl** builder, it applies the same RDFS-schema-aware primary key strategy — using `mRID` as the natural `[Key]` for `IdentifiedObject` subclasses and a surrogate UUID `id` for non-`IdentifiedObject` classes — producing a C# persistence layer in direct structural alignment with the generated SQL DDL schema. |
...
| [jpa-rdfs.xsl](shipped-builders/jpa-rdfs/builder.md) | Java source file | The **[jpa-rdfs.xsl](shipped-builders/jpa-rdfs/builder.md)** builder generates a Java source file (`*.jpa-rdfs.java`) containing RDFS-schema-aware [Jakarta Persistence 3.1](https://jakarta.ee/specifications/persistence/3.1/) (Jakarta EE 10) entity classes. Designed as a companion to the **sql-rdfs-ansi92.xsl** builder, it uses `mRID` as the natural `@Id` for `IdentifiedObject` subclasses and a surrogate UUID `id` for non-`IdentifiedObject` classes, producing a JPA persistence layer in direct structural alignment with the generated SQL DDL schema. |
| [jpa.xsl](shipped-builders/jpa/builder.md) | Java source file | The **[jpa.xsl](shipped-builders/jpa/builder.md)** builder generates a Java source file (`*.java`) containing JPA classes compatible with JPA 2.2 and earlier. The defined JPA classes can be utilized in Java applications for persisting profiles to a database. |
...
| [sql-rdfs-ansi92.xsl](shipped-builders/sql-rdfs-ansi92/builder.md) | SQL DDL file | The **[sql-rdfs-ansi92.xsl](shipped-builders/sql-rdfs-ansi92/builder.md)** builder generates an ANSI SQL-compliant DDL script (`*.rdfs-ansi92.sql`) that creates the database tables and relationships corresponding to the classes, attributes, and associations defined in a profile. Unlike the **sql.xsl** builder, it applies RDFS-schema-aware primary key logic: `IdentifiedObject` subclasses use `mRID` as a natural `VARCHAR(100)` primary key, while non-`IdentifiedObject` classes receive a surrogate UUID `id` column with a heuristic `UNIQUE` constraint. Designed to be used with the companion **jpa-rdfs.xsl** and **csharp-ef-rdfs.xsl** builders. |
| [sql.xsl](shipped-builders/sql/builder.md) | SQL DDL file | The **[sql.xsl](shipped-builders/sql/builder.md)** builder generates an ANSI SQL-compliant DDL (Data Definition Language) script (`*.sql`) that can be used to create the database tables and fields corresponding to the classes and attributes represented in a profile. |
