...
- **Domain** elements - represent attributes that have a type defined that corresponds to a &lt;&lt;CIMDatatype&gt;&gt; class in the CIM. These types resolve to a primitive and therefore the XML child element will contain an @xstype attribute to specify the primitive.
...
Turning now to developing and testing your builder. Provided in the ```/builder-submissions``` folder is the **CIMTool-Test-Project**. This project contains two ```.owl``` profiles along with their respective ```.xml``` files. The **EndDeviceControlsTestProfilWithAnonymousTypes.owl** profile and its corresponding [EndDeviceControlsTestProfilWithAnonymousTypes.xml](builder-submissions/CIMTool-Test-Project/Profiles/EndDeviceControlsTestProfilWithAnonymousTypes.xml) contain all possible XML elements as just described while the **EndDeviceControlsTestProfile.owl** and its corresponding [EndDeviceControlsTestProfile.xml](builder-submissions/CIMTool-Test-Project/Profiles/EndDeviceControlsTestProfile.xml) file are defined without anonymous type definitions as previously described. The project has been provided as a matter of convenience. You can use it to create your own profile definitions or utilize either of the pre-generated ```.xml``` files for testing your builder depending on whether your builder's target output supports anonymous classes.
...
For copyright support you must determine which of the two your builder will use.  Most likely your builder will utilize the multiline **copyright**. Refer to existing builders in this library for examples of how these copyright parameters are used. Currently only the JSON schema related builders utilize the **copyright-single-line** as JSON has certain constraints related to carriage returns in descriptions and comments within schemas.

### Step 4:  Final Pre-Commit Checklist for Review

Once you've determined your builder is ready confirm the following items:

1. The detailed description in your ```builder.md``` has been completed.  If there are particular aspects of your builder that should be highlighted this should be done here. For example, if your builder warrants it, you may include images in your builder's folder and include links to them in your description.  A great example of this is The [profile-doc-rtf.xsl builder](shipped-builder/profile-doc-rtf/profile-doc-rtf.xsl).
...
4. You have added a new row to the **Community Developed Builders** table in this README. The description in the row entry is typically a simplified version of the description in the builder's ```builders.md``` page.
