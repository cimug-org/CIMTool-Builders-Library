...
Provided in the ```/builder-submissions``` folder of this repository is a **CIMTool-Test-Project** which contains a profile called **EndDeviceControlsTestProfile.owl** along with it's corresponding [EndDeviceControlsTestProfile.xml](builder-submissions/CIMTool-Test-Project/Profiles/EndDeviceControlsTestProfile.xml) file. This test profile contains all the possible XML elements that your XSLT transform builder may want to account for. Many builders handle all of them with some builders ignoring in-lined anonymous definitions. For example, the XSD and JSON schema builders support them since there are ways in those respective specifications for defining such in-line type definitions. However, the JPA builder does not.
