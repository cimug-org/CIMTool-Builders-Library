		...
		<xsl:if test="$generate_jsonschema2pojo_annotations = 'true'">
			...
			<xsl:if test="$super">
				<list begin="&quot;extends&quot;: {{" indent="    " delim="," end="}}">
                    <item>"$ref": "#/definitions/<xsl:value-of select="$super/@name" />"</item>
                    <!--
					...
					<list begin="&quot;items&quot;: {{" indent="    " delim="," end="}}">
						<xsl:choose>
							<xsl:when test="count(self::a:Choice[./a:Reference]) > 0">
								<item>"$ref": "#/definitions/<xsl:value-of select="concat(@inheritanceBaseType, 'Ref')" />"</item>
							</xsl:when>
							<xsl:otherwise>
								<item>"$ref": "#/definitions/<xsl:value-of select="@inheritanceBaseType" />"</item>
							</xsl:otherwise>
						</xsl:choose>
					</list>
					<xsl:if test="@minOccurs &gt;= 1">
