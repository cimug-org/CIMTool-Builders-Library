# adoc-article-xsd.xsl

This builder currently ships with CIMTool.

## Builder Description

The **[adoc-article-xsd.xsl](adoc-article-xsd.xsl)** builder generates an [AsciiDoc](https://asciidoc.org/) file containing documentation for all classes, attributes, and associations defined in a **CIMTool** profile. The generated output is a complete, standalone document with a full document header, suitable for direct publication using the AsciiDoc toolchain or conversion to HTML, PDF, and other output formats via tools such as [Asciidoctor](https://asciidoctor.org/). The documentation content is structured around XSD schema concepts.

This builder is the article-doctype counterpart to the **[adoc-inline-xsd.xsl](../adoc-inline-xsd/builder.md)** builder.
...
The recommended best practice is to rename the builder file  (from ```adoc-article-xsd.xsl``` to ```custom-adoc-article-xsd.xsl``` for example) before importing. On import this will be interpreted as a new builder and you will be allowed to enter values in the Type and Extension fields.
...
| Field | Value |
|---|---|
| Builder XSLT Name: | `adoc-article-xsd.xsl` |
| Includes File Name: |  |
| Transform Builder Type: | `TEXT` |
| File Extension of Generated Files: | `article-xsd.adoc` |
